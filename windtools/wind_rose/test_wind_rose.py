...
def test_one_in_each():
    binned_data = WindRose.BuildStatTable(data_one, bins_one, num_dir_bins=4)
    assert np.all(binned_data == (1.0 / len(binned_data.flat) * 100)) 

def test_add_to_100percent():
    binned_data = WindRose.BuildStatTable(data_norm, bins, num_dir_bins=16)
    print binned_data
...
def test_missing_data():
    ...
    binned_data = WindRose.BuildStatTable(data_norm, bins, num_dir_bins=4)
    print binned_data
    print binned_data.sum()
    assert round(binned_data.sum(), 10)  == 100.0

def test_dir_avg():
    avg = WindRose.BuildDirAverages(data_one, num_dir_bins=4)
...
def test_dir_nan():
    ...
    data [2,1] = np.nan
    data [3,0] = np.nan
    avg = WindRose.BuildDirAverages(data, num_dir_bins=4)
