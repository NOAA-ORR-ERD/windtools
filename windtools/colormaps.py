...
get_color(value_list, range=(0,255), out_type='int')
  ...
  value_list: is a list (or any sequence) of values that you want a color to correspond to.
...
map_list: is a 256 element list of RGB triple tuples, Each R,G,B value is an integer
          from 0 to 255.
...
class ColorMap:
    def __init__(self, MapName=None, map_list=None, ValRange=(0, 255)):
        if MapName is not None:
            ...
            raise Exception("ColorMap must get either MapName or map-list passed in")
    ...
    def get_colors(self, values, ValRange=None, out_type = 'int'):
        ...
        if ValRange == (0, 255):
            ...
            values = ( (values - ValRange[0]) / (ValRange[1]-ValRange[0]) * 255)
            values.clip(min=0, max=255, out=values) # force out-of-range values into the acceptable range for map_list
...
if __name__ == "__main__":
    ...
    def ShowColorMaps():
        import wx
        # an app to show the colormaps
        class TestFrame(wx.Frame):
            def __init__(self, *args, **kwargs):
                wx.Frame.__init__(self, *args, **kwargs)
                Hsizer = wx.BoxSizer(wx.HORIZONTAL)
                for name in NamedColorMaps:
