...
class ReaderTester:
    ...
    def __init__(self):
        ...
        if self.filename is Undefined:
            raise nose.SkipTest
        self.MD = MetData.MetData( os.path.join(TestDataDir, self.filename) )
    ...
    def test_units(self):
        if self.units is Undefined:
            raise nose.SkipTest
        for key,value in self.units.items():
            assert_equal( self.MD.Units[key], value)

    def test_time_zone(self):
        if self.time_zone is Undefined:
            raise nose.SkipTest
...
def test_BrokenPorts():
    ...
    assert_raises(MetData.NoFileTypeMatchError,
                  MetData.MetData, os.path.join(TestDataDir, filename) )
